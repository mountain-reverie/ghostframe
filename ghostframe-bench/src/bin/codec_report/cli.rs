...
#[command(
    name = "codec_report",
    about = "M3.5 Layer B bench: run scenes, emit markdown report."
)]
pub struct Cli {
    ...
    #[arg(
        long,
        default_value = "solid,flat_ui,text,gradient,photo,motion,mode_switch",
        value_delimiter = ','
    )]
...
fn parse_duration(s: &str) -> Result<Duration, String> {
    ...
    match unit {
        ...
        other => Err(format!(
            "unknown duration unit: {other:?} (accepted: s, ms, m)"
        )),
    }
}
