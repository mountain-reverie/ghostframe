...
fn codec_report_smoke() {
    ...
    assert!(text.contains("## 2. End-to-end latency per scene"), "section 2 header missing");
    assert!(text.contains("## 3. Wire bandwidth"), "section 3 header missing");
    assert!(text.contains("## 4. Per-codec micro-bench"), "section 4 header missing");
    assert!(text.contains("## 5. Per-codec compressed size"), "section 5 header missing");
