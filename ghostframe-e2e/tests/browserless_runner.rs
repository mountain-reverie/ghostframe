...
async fn retransmits_fire_under_loss_but_not_on_a_perfect_link() {
    async fn run_at(net: NetProfile, seed: u64) -> (u64, u64, u64, u64) {
        let scene = BrowserlessScene {
            seed,
            // Overdrive on purpose. This test proves the retransmit path is
            // *reachable*; it does not claim the path is used in normal
            // operation. Once the budget floor became one datagram the
            // scheduler paces itself, and on a busy grid a dropped tile is
            // superseded by the next generation before any repair is due --
            // measured nack_hit=0 rto_fired=0 against bytes_dropped=3494.
            // That is correct behaviour (repairing superseded content is
            // waste), but it leaves this assertion with nothing to observe,
            // so the old flat 256 KiB floor is pinned here to keep the
            // contrast meaningful.
            //
            // The safety property this does *not* cover is covered
            // separately and still holds under the shipped default: see
            // `a_solid_tile_whose_only_datagram_is_dropped_is_still_repaired`,
            // where the tile is never superseded and must be repaired.
            tick_budget_floor_bytes: Some(256 * 1024),
            datagram_send_buffer_bytes: None,
