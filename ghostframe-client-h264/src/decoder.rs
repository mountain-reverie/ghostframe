...
mod tests {
    ...
    /// The owner's "renew the decoder with the right size, gracefully"
    /// requirement (design doc §10.7/§11), at the decoder level: a single
    /// `H264Decoder`, never told any max size ahead of time, must keep
    /// decoding correctly across a resolution change mid-session. This is
    /// ffmpeg's own default behaviour -- `get_format` runs again on the new
    /// SPS (`h264_slice.c`'s `must_reinit`), `hw_frames_ctx` stays null, and
    /// `ff_decode_get_hw_frames_ctx` builds a fresh, correctly-sized
    /// `AVHWFramesContext` on demand -- not something this crate implements;
    /// this test exists to prove it, not assume it.
    ///
    /// Calls `finish()` + `reset()` between the two resolutions -- not
    /// something `Renderer`'s real resolution-change path does (see
    /// `h264_resolution_change_timing` in `gpu_h264_render.rs`, which found
    /// no `frame_seq` gap at a resolution change and so no `reset()` call)
    /// -- purely so the per-frame dimension assertions below cannot be
    /// flaked by a B-frame-reordered picture from the old resolution's clip
    /// arriving after the new resolution's frames start. `reset()`'s own
    /// doc already names "a resolution change" as one of its intended uses.
    #[test]
    fn decoder_survives_a_mid_stream_resolution_change() {
        if skip_without_vaapi() {
            return;
        }
        let mut dec = H264Decoder::new().expect("open decoder");

        let clip_a = gradient_clip(640, 480, 3);
        let mut frames = 0;
        for au in &clip_a {
            for frame in dec.decode(au).expect("decode at 640x480") {
                assert_eq!(frame.width(), 640);
                assert_eq!(frame.height(), 480);
                frames += 1;
            }
        }
        frames += dec.finish().expect("finish at 640x480").len();
        dec.reset();

        let clip_b = gradient_clip(1280, 720, 3);
        for au in &clip_b {
            for frame in dec.decode(au).expect("decode at 1280x720") {
                assert_eq!(frame.width(), 1280);
                assert_eq!(frame.height(), 720);
                frames += 1;
            }
        }
        frames += dec.finish().expect("finish at 1280x720").len();

        assert_eq!(
            frames, 6,
            "must decode every frame across the resolution change: 3 before + 3 after"
        );
    }
}
