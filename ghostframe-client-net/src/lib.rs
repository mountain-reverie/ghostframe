...
impl ClientNet {
    ...
    fn drain_connection_events(&mut self, now_us: u64) {
        ...
        // on a bidirectional feedback stream on the real client) is out of
        // scope for the datagram path wired up here, so it is drained and
        // dropped rather than left queued — leaving it in place would
        // block every `Datagram` output behind it in the same `VecDeque`.
        //
            ...
            if let Some(session_stream) = self.handshake.session_stream() {
                ...
                while let Some(output) = self.core.poll_transmit(now_us) {
                    match output {
                        ...
                        PollOutput::Stream(_bytes) => {
                            // Feedback-stream wiring is a later task's concern.
                        }
                    }
                }
            }
